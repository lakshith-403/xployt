...
//             const result = await this.projectsCache.load(userId);
//             this.projects = Array.isArray(result[0]) ? result[0] : result; // Flatten if nested array
//         } catch (err) {
...
//                 QuarkFunction(row, 'td', '', {}, project.id.toString()); // Convert id to string
//                 QuarkFunction(row, 'td', '', {}, project.title);
//                 QuarkFunction(row, 'td', '', {}, project.state); // Changed from status to state
