...
export class ValidatorDashboard extends View {
  ...
  private async loadProjectStats(): Promise<any> {
    try {
      const response = await NETWORK.get(`/api/dashboard/project-stats/validator/${this.userId}`, { showLoading: true, handleError: true, throwError: true });
  ...
  private async loadRecentProjects(): Promise<any> {
    try {
      const response = await NETWORK.get(`/api/dashboard/recent-projects/validator/${this.userId}`, { showLoading: true, handleError: true, throwError: true });
      console.log(response);
  ...
  async render(q: Quark): Promise<void> {
    ...
    $(q, 'div', 'py-2 d-flex flex-column align-items-center', {}, (q) => {
      $(q, 'h1', 'text-center heading-1', {}, 'Validator Dashboard');
...
// export const validatorDashboardViewHandler = new ViewHandler('/validator-dashboard', ValidatorDashboard);
